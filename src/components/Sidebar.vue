    ...
    <div class="list-conversations description">
        <div class="conversation" v-for="(item, index) in rooms" :id-room="index" ref="conversation" @click="onClick">
            <div class="description">
                ...
                <i class="setting" @click="() => { initialVisibleModal = true }" >
...
<!--    {{modalBody = <CAvatar text-color="white" color="dark" status="success">{{ user.username[0] }}</CAvatar>}}-->
    <Modal
            :initialVisibleModal="initialVisibleModal"
            :initialHeaderTitle="'Conversation setting'"
            :initialContentBody="'body text'"
            @showHidenModal="showHidenModal">
        <template v-slot:bodySlot="iconProps">
            <input ref="test" type="text" class="form-control" v-model="testinput" @change="iconProps.change" />
        </template>
    </Modal>
